...
public class fragment_events_created extends Fragment implements Interfaz{
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        con.execute("http://10.4.41.144:3000/participant/notRated/" + UsuariSingleton.getInstance().getId(), "GET", null);
        ...
        try {
            if (datos.getInt("codigo") == 200) {
                ...
                for(int i = 0; i < response.length(); i++) {
                    ...
                    listEventsCreated.add(new holder_event_card(titulo, creador, calendario));
                }
                events_recycleview_adapter recyclerAdapter = new events_recycleview_adapter(getContext(), listEventsCreated);
                recyclerView.setAdapter(recyclerAdapter);
                //OperacionAuxiliar();
